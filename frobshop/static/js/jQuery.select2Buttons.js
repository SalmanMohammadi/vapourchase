...
var selectName = $('.simple-select').attr('name');
alert(selectName);

// add a hidden element with the same name as the select
var hidden = $('<input type="hidden" name="'+selectName+'">');
hidden.val($('.simple-select').val());
hidden.insertAfter($('.simple-select'));

$(".simple-select option").unwrap().each(function() {
    var selectbtn = $('<div class="selectbtn">'+$(this).text()+'</div>');
    if($(this).is(':checked')) selectbtn.addClass('on');
    $(this).replaceWith(selectbtn);
});
